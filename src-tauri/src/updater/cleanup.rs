...
pub fn cleanup_incomplete_downloads() -> Result<(), String> {
    log_error("Starting environment cleanup...");
        ...
        if path.is_file() {
            if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                if extensions.contains(&ext) {
                    log_error(&format!("Removing incomplete file: {:?}", path));
            ...
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                if name.starts_with("sf-") || name.ends_with(".tmp") {
                    log_error(&format!("Removing temp file: {:?}", path));
                    let _ = fs::remove_file(path.clone());
                }
            }
        }
...
fn clean_incomplete_game(game_dir: &Path) -> Result<(), String> {
    ...
    if staging_dir.exists() {
        log_error("Found leftover staging directory, cleaning up...");
