...
export default function ModBrowser({ installedMods, onInstallRequest, installingIds }: ModBrowserProps) {
    ...
    }, [query, page, loading, sortField, sortOrder]);
    ...
    return (
        <div className="flex flex-col h-full bg-[#121418]">
            <div className="p-6 border-b border-white/5 bg-[#1a1d23]/50 backdrop-blur-md sticky top-0 z-20">
                    ...
                    <div className="flex gap-4">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full bg-black/20 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-emerald-500/50 transition-colors"
                            />
                        </div>
                        <button
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {SORT_OPTIONS.map(opt => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${sortField === opt.id
                                ...
                                <div>
                                    ...
                                    <div className="flex bg-white/5 rounded-lg p-1 w-fit">
                                        <button
                                            ...
                                            className={`px-4 py-1.5 rounded-md text-sm transition-all ${sortOrder === 'desc'
                                        ...
                                        <button
                                            ...
                                            className={`px-4 py-1.5 rounded-md text-sm transition-all ${sortOrder === 'asc'
                                                ? 'bg-emerald-500 text-white shadow-md'
                                                : 'text-white/50 hover:text-white'
                                                }`}
                                        >
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    {loading && (
                        <div className="py-12 flex justify-center">
                            <Loader2 className="animate-spin text-emerald-500 text-3xl" />
                        </div>
                    )}
                    ...
                    {!loading && mods.length < total && mods.length > 0 && (
                        <div className="py-8 text-center">
                            <button
                                ...
                                className="px-6 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-white/70 transition-colors text-sm"
                            >
                                {t('mods.load_more')}
                            </button>
                        </div>
